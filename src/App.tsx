...
function App() {
  // Al iniciar, se intenta cargar el historial del chat desde localStorage
  const [messages, setMessages] = useState<Message[]>(() => {
    const saved = localStorage.getItem('messages');
    if (saved) {
      try {
        // Convertimos el JSON a objeto y rehidratamos la fecha
  ...
  // Cada vez que se actualiza 'messages', se guardan en localStorage
  useEffect(() => {
    localStorage.setItem('messages', JSON.stringify(messages));
  }, [messages]);

  // Actualiza la clase 'dark' en el documento y guarda el modo en localStorage
  useEffect(() => {
